...
// (both allow and deny), receiving the key and the resulting Result reported
// under the "adaptive" algorithm. The default is nil (a cheap no-op guarded by
// a nil check on the hot path). The hook runs synchronously on the calling
